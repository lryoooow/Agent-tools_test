...
async def test_detect_runner_can_run_when_rs_tools_switch_is_disabled(monkeypatch, tmp_path: Path) -> None:
    ...
    async def ok_client(self, tool_name, *, source_path, output_dir, arguments=None):
        ...
        return {
            ...
            "score_threshold": 0.5,
            "classes": [
                {"name": "ship", "label": "ship", "count": 2, "color": "#008080"},
            ],
        }
    ...
    assert geo["type"] == "detection"
    assert geo["imagery_id"] == imagery_id
    assert geo["result_url"] == f"/api/imagery/{imagery_id}/results/detection_overlay.png"
    assert geo["bounds"] == (100.0, 19.98, 100.02, 20.0)
    assert geo["detection_count"] == 2
    assert geo["score_threshold"] == 0.5
    assert geo["classes"] == [
        {"name": "ship", "label": "ship", "count": 2, "color": "#008080"},
    ]
...
@pytest.mark.asyncio
async def test_detect_runner_unexpected_exception_returns_unexpected_error(monkeypatch, tmp_path: Path) -> None:
    _prepare_imagery(tmp_path)
    monkeypatch.setenv("IMAGERY_UPLOAD_DIR", str(tmp_path))
    monkeypatch.setenv("RS_DETECT_MCP_USE_DOCKER", "true")
    get_settings.cache_clear()
